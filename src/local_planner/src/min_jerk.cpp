...
void minimumJerkTrajGen(
    ...
    const Eigen::Vector2d &initialVel,
    const Eigen::Vector2d &initialAcc,
    const Eigen::Vector2d &terminalPos,
    const Eigen::Vector2d &terminalVel,
    const Eigen::Vector2d &terminalAcc,
    const Eigen::Matrix2Xd &intermediatePositions,
    const Eigen::VectorXd &timeAllocationVector,
    // Outputs:
    Eigen::MatrixX2d &coefficientMatrix)
{
    ...
    //  - b matrix
    Eigen::MatrixXd b_pos = Eigen::MatrixXd::Zero(2 * pieceNum, 2);
    ...
    // - b matrix
    Eigen::MatrixXd b_vel = Eigen::MatrixXd::Zero(pieceNum + 1, 2);
    ...
    // - b matrix
    Eigen::MatrixXd b_acc = Eigen::MatrixXd::Zero(pieceNum + 1, 2);
    ...
    Eigen::MatrixXd b = Eigen::MatrixXd::Zero(4 * pieceNum + 2, 2);
    ...
    b.block(0, 0, 2 * pieceNum, 2) = b_pos;
    b.block(2 * pieceNum, 0, pieceNum + 1, 2) = b_vel;
    b.block(3 * pieceNum + 1, 0, pieceNum + 1, 2) = b_acc;

    //  -- P and q matrix for the KKT conditions
    Eigen::MatrixXd P = Eigen::MatrixXd::Zero(10 * pieceNum + 2, 10 * pieceNum + 2);
    Eigen::MatrixXd q = Eigen::MatrixXd::Zero(10 * pieceNum + 2, 2);
...
public:
    MinJerk(const Config &conf,
             ros::NodeHandle &nh_)
        : config(conf),
          nh(nh_),
          visualizer(nh),
          positions(2, config.maxPieceNum + 1),
          times(config.maxPieceNum),
          positionNum(0)
    {
        targetSub = nh.subscribe(config.targetTopic, 2,
                                 &MinJerk::targetCallBack, this,
    ...
    void targetCallBack(const nav_msgs::Path::ConstPtr &msg)
    {
        ...
        else if (positionNum > config.maxPieceNum + 1)
        {
            positionNum = config.maxPieceNum + 1;
        }

        positions = Eigen::MatrixXd::Zero(2, positionNum);
        times = Eigen::VectorXd::Zero(positionNum - 1);

        for (int i = 0; i < positionNum; i++)
        {
            positions(0, i) = msg->poses[i].pose.position.x;
            positions(1, i) = msg->poses[i].pose.position.y;
        }
        ...
        const Eigen::Vector2d terminalVel(config.terminalVel[0], config.terminalVel[1]);
        const Eigen::Vector2d terminalAcc(config.terminalAcc[0], config.terminalAcc[1]);
        const Eigen::Matrix2Xd intermediatePositions = positions.middleCols(1, pieceNum - 1);
        const Eigen::VectorXd timeAllocationVector = times.head(pieceNum);

        Eigen::MatrixX2d coefficientMatrix = Eigen::MatrixXd::Zero(6 * pieceNum, 2);
        ...
        for (int i = 0; i < pieceNum; i++)
        {
            traj.emplace_back(timeAllocationVector(i),
                                coefficientMatrix.block<6, 2>(6 * i, 0).transpose().rowwise().reverse());
        }
...
{
    ros::init(argc, argv, "min_jerk_node");
    ros::NodeHandle nh_;
    MinJerk MinJerk(Config(ros::NodeHandle("~")), nh_);
    ros::spin();
    return 0;
}
